...
uniform vec2 scaleXAndY;
uniform float scaleProportional;
uniform bool scaleProportionnalButton;
...
void main()
{
   ...
   //This is the condition to have the option to enable a proportional scaling for both X and Y, in a single numeric.
   if ( scaleProportionnalButton )
   {
   
   //Here we use a float, so both channels will be scaled by the same numeric.
   position_coords = (coords-0.5)/(scaleProportional/100.0)+ vec2(0.5);
   
   } else {
   
   //Here we use a vec2, so both channels will have their own numerics.
   position_coords = coords/(scaleXAndY/100.0);
